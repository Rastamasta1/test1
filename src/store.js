// src/store.js
// Pure ES module: vinyl record store with add, list, remove, find, count, search.
 ...
 * Returns all records whose name contains the query string.
 * An empty query matches all records.
 * @param {string} query
 * @returns {Array} Matching records (shallow copies).
 */
...
// When run directly (node src/store.js), print all records and demo search.
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  // Demo: search for records whose name contains 'blue' — matches some but not all
  const results = search('blue');
  console.log(`search('blue') found ${results.length} record(s):`);
