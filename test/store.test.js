...
import { add, list, remove, find, count, validate, search } from '../src/store.js';
...
test('search: returns records whose name contains the query', () => {
  // "Blue" matches "Kind of Blue" (rec-001) and "Blue" (rec-006)
  const results = search('Blue');
  ...
  assert.ok(results.every(r =>
    ...
    r.description.toLowerCase().includes('blue')
  ));
});

test('search: does not return records that do not match', () => {
  // "jazz" should not match records with no jazz reference
  const results = search('jazz');
  for (const r of results) {
    const matched =
      r.name.toLowerCase().includes('jazz') ||
      r.description.toLowerCase().includes('jazz');
    assert.ok(matched, `record ${r.id} should not be in jazz results`);
  }
});

test('search: is case-insensitive', () => {
  const lower = search('miles');
  const upper = search('MILES');
  const mixed = search('MiLeS');
  // All three should return the same set of ids
  const ids = r => r.map(x => x.id).sort();
  assert.deepEqual(ids(lower), ids(upper));
  assert.deepEqual(ids(lower), ids(mixed));
});

test('search: matches substring in description', () => {
  // "Coltrane" appears in descriptions but not necessarily in names
  const results = search('Coltrane');
  assert.ok(results.length >= 1);
  assert.ok(results.every(r =>
    r.name.toLowerCase().includes('coltrane') ||
    r.description.toLowerCase().includes('coltrane')
  ));
});

test('search: returns all records for empty string', () => {
  const results = search('');
  assert.equal(results.length, count());
});

test('search: returns all records for null', () => {
  const results = search(null);
  assert.equal(results.length, count());
});

test('search: returns all records for undefined', () => {
  const results = search(undefined);
  assert.equal(results.length, count());
});

test('search: returns empty array for query with no matches', () => {
  const results = search('xyzzy_no_match_12345');
  assert.deepEqual(results, []);
});

test('search: returns defensive copies (mutating result does not affect store)', () => {
  const results = search('Blue');
  const originalName = results[0].name;
  results[0].name = '__MUTATED__';
  const results2 = search('Blue');
  assert.equal(results2[0].name, originalName);
});

test('search: query matching some but not all records returns a proper subset', () => {
  // "1959" appears in Kind of Blue's description — a very specific year
  const results = search('1959');
  assert.ok(results.length >= 1, 'should find at least one record');
  assert.ok(results.length < count(), 'should not return all records');
  assert.ok(results.every(r =>
    r.name.toLowerCase().includes('1959') ||
    r.description.toLowerCase().includes('1959')
  ));
});
